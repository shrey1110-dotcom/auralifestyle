...
export default function ProductCard({ product }) {
  ...
  useEffect(() => {
    let on = true;
    if (!sku) return setStock(null);
    fetchStock(sku).then((s) => on && setStock(Number(s || 0)));
    return () => { on = false; };
  }, [sku]);
  const soldOut = stock === 0;
  const fewLeft = typeof stock === "number" && stock > 0 && stock <= 2;

  const onAdd = () => {
    if (soldOut) {
      show?.("Out of stock", { type: "error", timeout: 1500 });
      return;
    }
    addToCart?.({
      id: product.id,
  ...
  return (
    <div className="group rounded-xl overflow-hidden border hover:shadow-sm transition bg-white dark:bg-neutral-900 relative">
      {/* Stock badge */}
      {soldOut ? (
        <span className="absolute top-2 left-2 z-10 text-xs font-semibold px-2 py-1 rounded-md bg-rose-600 text-white">
          OUT OF STOCK
        </span>
      ) : fewLeft ? (
        <span className="absolute top-2 left-2 z-10 text-xs font-semibold px-2 py-1 rounded-md bg-amber-500 text-white">
          Only {stock} left
        </span>
      ) : null}
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            {typeof stock === "number" && !soldOut ? (
              <div className="text-xs mt-1 opacity-70">{stock} in stock</div>
            ) : null}
          </div>
        ...
        <div className="mt-2 flex items-center justify-between">
          ...
          <button
            ...
            className={`h-9 px-3 rounded-md text-sm inline-flex items-center gap-1.5
              ${soldOut ? "bg-neutral-300 text-neutral-600 cursor-not-allowed" : "bg-neutral-900 text-white dark:bg-white dark:text-neutral-900"}`}
          >
            ADD
            <svg className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
